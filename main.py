...
def main():
    print("Pleas input you data now")
    ...
    final_ramp_velocity: float = np.sqrt(2*9.8*(np.sin(angle)-coef_rolling_friction*np.cos(angle))*length)
    delta_time_1: float = (final_ramp_velocity*np.sin(angle)+np.sqrt((final_ramp_velocity**2)+2*9.8*inital_height))/(-9.8)
    delta_time_2: float = (final_ramp_velocity*np.sin(angle)-np.sqrt((final_ramp_velocity**2)+2*9.8*inital_height))/(-9.8)
